...
local function gen(ftype, vary_hunger, vary_thirst)
  local fgen = function(args)
    ...
    local name = cRP.getItemName(idname)

    choices[act] = {function(player,choice)
      local user_id = cRP.getUserId(player)
      if user_id ~= nil then
        if cRP.tryGetInventoryItem(user_id,idname,1,false) then
          if vary_hunger ~= 0 then cRP.varyHunger(user_id,vary_hunger) end
          if vary_thirst ~= 0 then cRP.varyThirst(user_id,vary_thirst) end

          if ftype == "drink" then
            cRPclient._notify(player,"~b~ Drinking "..name..".")
            play_drink(player)
          elseif ftype == "eat" then
            cRPclient._notify(player,"~o~ Eating "..name..".")
            play_eat(player)
          end
