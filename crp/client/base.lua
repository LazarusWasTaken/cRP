...
local Tunnel = module("cRP", "lib/Tunnel")
local Proxy = module("cRP", "lib/Proxy")
...
Tunnel.bindInterface("cRP", tcRP)

-- get server interface
cRPserver = Tunnel.getInterface("cRP")

-- add client proxy interface (same as tunnel interface)
Proxy.addInterface("cRP",tcRP)

-- functions

local user_id
function tcRP.setUserId(_user_id)
  user_id = _user_id
end

-- get user id (client-side)
function tcRP.getUserId()
  return user_id
end

function tcRP.teleport(x,y,z)
  tcRP.unjail() -- force unjail before a teleportation
  SetEntityCoords(GetPlayerPed(-1), x+0.0001, y+0.0001, z+0.0001, 1,0,0,1)
  cRPserver._updatePos(x,y,z)
end

-- return x,y,z
function tcRP.getPosition()
  local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
  return x,y,z
end

-- return false if in exterior, true if inside a building
function tcRP.isInside()
  local x,y,z = tcRP.getPosition()
  return not (GetInteriorAtCoords(x,y,z) == 0)
end

-- return vx,vy,vz
function tcRP.getSpeed()
  local vx,vy,vz = table.unpack(GetEntityVelocity(GetPlayerPed(-1)))
  return math.sqrt(vx*vx+vy*vy+vz*vz)
end

function tcRP.getCamDirection()
...
function tcRP.addPlayer(player)
  players[player] = true
end

function tcRP.removePlayer(player)
  players[player] = nil
end

function tcRP.getPlayers()
  return players
end

function tcRP.getNearestPlayers(radius)
  local r = {}
...
function tcRP.getNearestPlayer(radius)
  local p = nil

  local players = tcRP.getNearestPlayers(radius)
...
function tcRP.notify(msg)
  SetNotificationTextEntry("STRING")
  AddTextComponentString(msg)
  DrawNotification(true, false)
end

function tcRP.notifyPicture(icon, type, sender, title, text)
    SetNotificationTextEntry("STRING")
...
function tcRP.playScreenEffect(name, duration)
...
-- name, see https://wiki.fivem.net/wiki/Screen_Effects
function tcRP.stopScreenEffect(name)
  StopScreenEffect(name)
end
...
function tcRP.playAnim(upper, seq, looping)
  if seq.task then -- is a task (cf https://github.com/ImagicTheCat/cRP/pull/118)
    tcRP.stopAnim(true)
...
function tcRP.playSpatializedSound(dict,name,x,y,z,range)
  PlaySoundFromCoord(-1,name,x+0.0001,y+0.0001,z+0.0001,dict,0,range+0.0001,0)
end

-- play sound
function tcRP.playSound(dict,name)
  PlaySound(-1,name,dict,0,0,1)
end

--[[
-- not working
function tcRP.setMovement(dict)
  if dict then
...
AddEventHandler("playerSpawned",function()
  TriggerServerEvent("cRPcli:playerSpawned")
end)

AddEventHandler("onPlayerDied",function(player,reason)
  TriggerServerEvent("cRPcli:playerDied")
end)
