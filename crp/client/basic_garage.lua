-- decorators
DecorRegister("cRP_owner", 3)
DecorRegister("cRP_vmodel", 3)

local veh_models = {}
local vehicles = {}

function tcRP.setVehicleModelsIndex(index)
  veh_models = index
...
-- veh: vehicle game id
-- return owner_user_id, vname (or nil if not managed by cRP)
function tcRP.getVehicleInfos(veh)
  if veh and DecorExistOn(veh, "cRP_owner") and DecorExistOn(veh, "cRP_vmodel") then
    local user_id = DecorGetInt(veh, "cRP_owner")
    local vmodel = DecorGetInt(veh, "cRP_vmodel")
...
function tcRP.spawnGarageVehicle(name,pos) -- one vehicle per vname/model allowed at the same time
  ...
  if vehicle == nil then
    ...
    if HasModelLoaded(mhash) then
      ...
      SetVehicleNumberPlateText(nveh, "P "..tcRP.getRegistrationNumber())
      Citizen.InvokeNative(0xAD738C3085FE7E11, nveh, true, true) -- set as mission entity
      SetVehicleHasBeenOwnedByPlayer(nveh,true)

      -- set decorators
      DecorSetInt(veh, "cRP_owner", tcRP.getUserId())
      DecorSetInt(veh, "cRP_vmodel", veh_models[name])
    ...
    tcRP.notify("This vehicle is already out.")
  end
end

function tcRP.despawnGarageVehicle(name)
  local vehicle = vehicles[name]
  if vehicle then
    -- remove vehicle
...
Citizen.CreateThread(function()
  ...
  for k,v in pairs(vehicles) do
    ...
    elseif v.pos then -- not valid, respawn if with a valid position
      print("[cRP] invalid vehicle "..v[1]..", respawning...")
      tcRP.spawnGarageVehicle(v[1], v[2], v.pos)
    end
  end
end)
--]]
...
function tcRP.getNearestVehicle(radius)
  local x,y,z = tcRP.getPosition()
...
function tcRP.tryOwnNearestVehicle(radius)
  local veh = tcRP.getNearestVehicle(radius)
  if veh then
    local user_id, vname = tcRP.getVehicleInfos(veh)
    if user_id and user_id == tcRP.getUserId() then
...
function tcRP.fixeNearestVehicle(radius)
  local veh = tcRP.getNearestVehicle(radius)
...
function tcRP.replaceNearestVehicle(radius)
  local veh = tcRP.getNearestVehicle(radius)
  if IsEntityAVehicle(veh) then
...
function tcRP.getVehicleAtPosition(x,y,z)
  x = x+0.0001
  y = y+0.0001
  z = z+0.0001
...
-- return ok,name
function tcRP.getNearestOwnedVehicle(radius)
  tcRP.tryOwnNearestVehicle(radius) -- get back network lost vehicles
...
-- return ok,x,y,z
function tcRP.getAnyOwnedVehiclePosition()
  for k,v in pairs(vehicles) do
...
-- return x,y,z
function tcRP.getOwnedVehiclePosition(name)
  local vehicle = vehicles[name]
  local x,y,z = 0,0,0
...
function tcRP.getOwnedVehicleHandle(name)
  local vehicle = vehicles[name]
  if vehicle then
    return vehicle[2]
  end
...
function tcRP.isInVehicle()
  local ped = GetPlayerPed(-1)
  return IsPedSittingInAnyVehicle(ped) 
end

-- vehicle commands
function tcRP.vc_openDoor(name, door_index)
  local vehicle = vehicles[name]
  if vehicle then
...
function tcRP.vc_closeDoor(name, door_index)
  local vehicle = vehicles[name]
  if vehicle then
...
function tcRP.vc_detachTrailer(name)
  local vehicle = vehicles[name]
  if vehicle then
...
function tcRP.vc_detachTowTruck(name)
  local vehicle = vehicles[name]
  if vehicle then
...
function tcRP.vc_detachCargobob(name)
  local vehicle = vehicles[name]
  if vehicle then
...
function tcRP.vc_toggleEngine(name)
  local vehicle = vehicles[name]
  if vehicle then
...
function tcRP.vc_toggleLock(name)
  local vehicle = vehicles[name]
  if vehicle then
    local veh = vehicle[2]
    ...
    if locked then -- unlock
      ...
      tcRP.notify("Vehicle unlocked.")
    else -- lock
      SetVehicleDoorsLocked(veh,2)
      SetVehicleDoorsLockedForAllPlayers(veh, true)
      tcRP.notify("Vehicle locked.")
    end
  end
end
