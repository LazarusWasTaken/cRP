...
function tcRP.addBlip(x,y,z,idtype,idcolor,text)
...
-- return native id
function tcRP.setNamedBlip(name,x,y,z,idtype,idcolor,text)
  tcRP.removeNamedBlip(name) -- remove old one

  named_blips[name] = tcRP.addBlip(x,y,z,idtype,idcolor,text)
  return named_blips[name]
end

-- remove a named blip
function tcRP.removeNamedBlip(name)
  if named_blips[name] ~= nil then
    tcRP.removeBlip(named_blips[name])
...
-- return marker id
function tcRP.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
...
-- return id
function tcRP.setNamedMarker(name,x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  tcRP.removeNamedMarker(name) -- remove old marker

  named_markers[name] = tcRP.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  return named_markers[name]
end

function tcRP.removeNamedMarker(name)
  if named_markers[name] then
    tcRP.removeMarker(named_markers[name])
...
function tcRP.setArea(name,x,y,z,radius,height)
...
-- locked: boolean
-- doorswing: -1 to 1
function tcRP.setStateOfClosestDoor(doordef, locked, doorswing)
...
function tcRP.openClosestDoor(doordef)
  tcRP.setStateOfClosestDoor(doordef, false, 0)
end

function tcRP.closeClosestDoor(doordef)
  tcRP.setStateOfClosestDoor(doordef, true, 0)
end
