...
function tcRP.playerStateReady(state)
  state_ready = state
end
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerPlaying(PlayerId()) and state_ready then
      ...
      cRPserver._updateHealth(tcRP.getHealth())
      cRPserver._updateWeapons(tcRP.getWeapons())
      cRPserver._updateCustomization(tcRP.getCustomization())
    end
  end
end)
...
function tcRP.getWeaponTypes()
  return weapon_types
end

function tcRP.getWeapons()
...
function tcRP.giveWeapons(weapons,clear_before)
  local player = GetPlayerPed(-1)
...
function tcRP.setArmour(amount)
  SetPedArmour(GetPlayerPed(-1), amount)
end

--[[
function tcRP.dropWeapon()
...
function tcRP.getDrawables(part)
...
function tcRP.getDrawableTextures(part,drawable)
  local isprop, index = parse_part(part)
  if isprop then
...
function tcRP.getCustomization()
...
function tcRP.setCustomization(custom) -- indexed [drawable,texture,palette] components or props (p0...) plus .modelhash or .model
  local r = async()
    ...
    if custom then
      ...
      if mhash then
        ...
        if HasModelLoaded(mhash) then
          ...
          local weapons = tcRP.getWeapons()
          local armour = GetPedArmour(ped)
          SetPlayerModel(PlayerId(), mhash)
          tcRP.giveWeapons(weapons,true)
          tcRP.setArmour(armour)
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(60000)
    if state_ready then
      local custom = tcRP.getCustomization()
      custom.model = nil
      custom.modelhash = nil
      tcRP.setCustomization(custom)
    end
  end
end)
--]]
