...
function tcRP.varyHealth(variation)
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerPlaying(PlayerId()) then
      ...
      if IsPedOnFoot(ped) and not tcRP.isNoclip() then
        local factor = math.min(tcRP.getSpeed(),10)
      ...
      -- do variation
      if vthirst ~= 0 then
        cRPserver._varyThirst(vthirst/12.0)
      end

      if vhunger ~= 0 then
        cRPserver._varyHunger(vhunger/12.0)
      end
    end
  end
end)
  ...
  while true do
    ...
    if health <= cfg.coma_threshold and coma_left > 0 then
      if not in_coma then -- go to coma state
        ...
        cRPserver._updateHealth(cfg.coma_threshold) -- force health update

        SetEntityHealth(ped, cfg.coma_threshold)
        SetEntityInvincible(ped,true)
        tcRP.playScreenEffect(cfg.coma_effect,-1)
        tcRP.ejectVehicle()
        tcRP.setRagdoll(true)
      else -- in coma
        -- maintain life
      ...
      if in_coma then -- get out of coma state
        ...
        tcRP.setRagdoll(false)
        tcRP.stopScreenEffect(cfg.coma_effect)
...
function tcRP.isInComa()
  return in_coma
end

-- kill the player if in coma
function tcRP.killComa()
  if in_coma then
    coma_left = 0
  end
end
