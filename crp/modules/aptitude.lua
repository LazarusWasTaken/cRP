...
function cRP.defAptitudeGroup(group, title)
  gaptitudes[group] = {_title = title}
end

-- max_exp: -1 => infinite
function cRP.defAptitude(group, aptitude, title, init_exp, max_exp)
...
function cRP.getAptitudeDefinition(group, aptitude)
  local vgroup = gaptitudes[group]
...
function cRP.getAptitudeGroupTitle(group)
...
function cRP.getUserAptitudes(user_id)
...
function cRP.varyExp(user_id, group, aptitude, amount)
  local def = cRP.getAptitudeDefinition(group, aptitude)
  local uaptitudes = cRP.getUserAptitudes(user_id)
  if def ~= nil and uaptitudes ~= nil then
    -- apply variation
    local exp = uaptitudes[group][aptitude]
    local level = math.floor(cRP.expToLevel(exp)) -- save level before variation
    ...
    if player ~= nil then
      local group_title = cRP.getAptitudeGroupTitle(group)
      local aptitude_title = def[1]

      --- exp
      if amount < 0 then
        cRPclient._notify(player,lang.aptitude.lose_exp({group_title,aptitude_title,-1*amount}))
      elseif amount > 0 then
        cRPclient._notify(player,lang.aptitude.earn_exp({group_title,aptitude_title,amount}))
      end
      --- level up/down
      local new_level = math.floor(cRP.expToLevel(exp))
      local diff = new_level-level
      if diff < 0 then
        cRPclient._notify(player,lang.aptitude.level_down({group_title,aptitude_title,new_level}))
      elseif diff > 0 then
        cRPclient._notify(player,lang.aptitude.level_up({group_title,aptitude_title,new_level}))
      end
    end
  end
end

function cRP.levelUp(user_id, group, aptitude)
  local exp = cRP.getExp(user_id,group,aptitude)
  local next_level = math.floor(cRP.expToLevel(exp))+1
  local next_exp = cRP.levelToExp(next_level)
  local add_exp = next_exp-exp
  cRP.varyExp(user_id, group, aptitude, add_exp)
end

function cRP.levelDown(user_id, group, aptitude)
  local exp = cRP.getExp(user_id,group,aptitude)
  local prev_level = math.floor(cRP.expToLevel(exp))-1
  local prev_exp = cRP.levelToExp(prev_level)
  local add_exp = prev_exp-exp
  cRP.varyExp(user_id, group, aptitude, add_exp)
end

function cRP.getExp(user_id, group, aptitude)
  local uaptitudes = cRP.getUserAptitudes(user_id)
...
function cRP.setExp(user_id, group, aptitude, amount)
  local exp = cRP.getExp(user_id, group, aptitude)
  cRP.varyExp(user_id, group, aptitude, amount-exp)
end

-- return float
function cRP.expToLevel(exp)
  return (math.sqrt(1+8*exp/exp_step)-1)/2
end

-- return integer
function cRP.levelToExp(lvl)
...
for k,v in pairs(cfg.gaptitudes) do
  cRP.defAptitudeGroup(k,v._title or "")
  for l,w in pairs(v) do
    if l ~= "_title" then
      cRP.defAptitude(k,l,w[1],w[2],w[3])
    end
  end
end
...
local function ch_aptitude(player,choice)
  ...
  if user_id then
    if player_apts[player] then -- hide
      ...
      cRPclient._removeDiv(player,"user_aptitudes")
    else -- show
      local content = ""
      local uaptitudes = cRP.getUserAptitudes(user_id)
      for k,v in pairs(uaptitudes) do
        -- display group
        content = content..lang.aptitude.display.group({cRP.getAptitudeGroupTitle(k)}).."<br />"
        for l,w in pairs(v) do
          local def = cRP.getAptitudeDefinition(k,l)
          if def then
            -- display aptitude
            local exp = uaptitudes[k][l]
            local flvl = cRP.expToLevel(exp)
...
.div_user_aptitudes .dprogressbar{
  ...
  height: 20px;
      ...
      cRPclient._setDiv(player,"user_aptitudes",css, content)
    end
  end
end
