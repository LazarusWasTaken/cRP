...
for gtype,weapons in pairs(gunshop_types) do
  ...
  local gunshop_choice = function(player,choice)
    ...
    if weapon then
      ...
      -- prompt amount
      local amount = cRP.prompt(player,lang.gunshop.prompt_ammo({choice}),"")
      ...
      if amount >= 0 then
        ...
        -- payment
        if user_id and cRP.tryPayment(user_id,total) then
          cRPclient._giveWeapons(player,{
            [weapon] = {ammo=amount}
          })

          cRPclient._notify(player,lang.money.paid({total}))
        else
  ...
  if user_id then
    for k,v in pairs(gunshops) do
      ...
      if group and menu then
        ...
        cRPclient._addBlip(source,x,y,z,gcfg.blipid,gcfg.blipcolor,lang.gunshop.title({gtype}))
        cRPclient._addMarker(source,x,y,z-1,0.7,0.7,0.5,0,255,125,125,150)

        cRP.setArea(source,"cRP:gunshop"..k,x,y,z,1,1.5,gunshop_enter,gunshop_leave)
      end
    end
  end
end
