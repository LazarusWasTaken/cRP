...
function cRP.removeCloak(player)
  local user_id = cRP.getUserId(player)
  if user_id then
    local data = cRP.getUserDataTable(user_id)
    if data then
      if data.cloakroom_idle ~= nil then -- consume cloakroom idle
        cRPclient._setCustomization(player,data.cloakroom_idle)
...
async(function()
  ...
  for k,v in pairs(cfg.cloakroom_types) do
    ...
    local choose = function(player, choice)
      ...
      if custom then
        old_custom = cRPclient.getCustomization(player)
        local idle_copy = {}
        ...
        cRPclient._setCustomization(player,idle_copy)
      end
    end

    -- rollback clothes
    if not not_uniform then
      menu[lang.cloakroom.undress.title()] = {function(player,choice) cRP.removeCloak(player) end}
    end
...
local function build_client_points(source)
  for k,v in pairs(cfg.cloakrooms) do
    ...
    if cloakroom and menu then
      ...
      cRP.setArea(source,"cRP:cfg:cloakroom"..k,x,y,z,1,1.5,cloakroom_enter,cloakroom_leave)
    end
  end
end
