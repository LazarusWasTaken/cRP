...
-- return group title
function cRP.getGroupTitle(group)
...
function cRP.getUserGroups(user_id)
...
function cRP.addUserGroup(user_id,group)
  if not cRP.hasGroup(user_id,group) then
    local user_groups = cRP.getUserGroups(user_id)
    ...
    if ngroup then
      ...
      TriggerEvent("cRP:playerJoinGroup", user_id, group, gtype)
    end
  end
end
...
function cRP.getUserGroupByType(user_id,gtype)
  local user_groups = cRP.getUserGroups(user_id)
  for k,v in pairs(user_groups) do
    local kgroup = groups[k]
    if kgroup then
...
function cRP.getUsersByGroup(group)
  local users = {}

  for k,v in pairs(cRP.rusers) do
    if cRP.hasGroup(tonumber(k),group) then table.insert(users, tonumber(k)) end
  end
...
function cRP.getUsersByPermission(perm)
  local users = {}

  for k,v in pairs(cRP.rusers) do
    if cRP.hasPermission(tonumber(k),perm) then table.insert(users, tonumber(k)) end
  end
...
function cRP.removeUserGroup(user_id,group)
  local user_groups = cRP.getUserGroups(user_id)
  ...
  TriggerEvent("cRP:playerLeaveGroup", user_id, group, gtype)

  user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function cRP.hasGroup(user_id,group)
  local user_groups = cRP.getUserGroups(user_id)
...
function cRP.registerPermissionFunction(name, callback)
  func_perms[name] = callback
end

-- register not fperm (negate another fperm)
cRP.registerPermissionFunction("not", function(user_id, parts)
  return not cRP.hasPermission(user_id, "!"..table.concat(parts, ".", 2))
end)

cRP.registerPermissionFunction("is", function(user_id, parts)
  local param = parts[2]
  if param == "inside" then
    local player = cRP.getUserSource(user_id)
    if player then
      return cRPclient.isInside(player)
    end
  elseif param == "invehicle" then
    local player = cRP.getUserSource(user_id)
    if player then
      return cRPclient.isInVehicle(player)
    end
  end
end)

-- check if the user has a specific permission
function cRP.hasPermission(user_id, perm)
  local user_groups = cRP.getUserGroups(user_id)
  ...
  if fchar == "@" then -- special aptitude permission
    ...
    if #parts == 3 then -- decompose group.aptitude.operator
      ...
      local alvl = math.floor(cRP.expToLevel(cRP.getExp(user_id,group,aptitude)))
  ...
  elseif fchar == "#" then -- special item permission
    ...
    if #parts == 2 then -- decompose item.operator
      ...
      local amount = cRP.getInventoryItemAmount(user_id, item)
...
function cRP.hasPermissions(user_id, perms)
  for k,v in pairs(perms) do
    if not cRP.hasPermission(user_id, v) then
      return false
    end
  end
...
for k,v in pairs(selectors) do
  ...
  local function ch_select(player,choice)
    ...
    if user_id then
      ...
      if gname then
        cRP.addUserGroup(user_id, gname)
  ...
  for l,w in pairs(v) do
    if l ~= "_config" then
      local title = cRP.getGroupTitle(w)
      kgroups[title] = w
  ...
  if user_id then
    for k,v in pairs(selectors) do
      ...
      if gcfg and menu then
        ...
        cRPclient._addBlip(source,x,y,z,gcfg.blipid,gcfg.blipcolor,k)
        cRPclient._addMarker(source,x,y,z-1,0.7,0.7,0.5,255,154,24,125,150)

        cRP.setArea(source,"cRP:gselector:"..k,x,y,z,1,1.5,selector_enter,selector_leave)
      end
    end
  end
end
  ...
  if first_spawn then
    ...
    if user then
      for k,v in pairs(user) do
        cRP.addUserGroup(user_id,v)
      end
    end

    -- add default group user
    cRP.addUserGroup(user_id,"user")
  end
