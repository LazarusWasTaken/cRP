...
function cRP.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  cRPclient._openMenuData(source, menudata)
end

-- force close player menu
function cRP.closeMenu(source)
  cRPclient._closeMenu(source)
end
...
-- return entered text
function cRP.prompt(source,title,default_text)
  local r = async()
  prompts[source] = r

  cRPclient._prompt(source, title,default_text)
...
function cRP.request(source,text,time)
  local r = async()
  ...
  cRPclient.request(source,id,text,time) -- send request to client
...
function cRP.registerMenuBuilder(name, builder)
...
function cRP.buildMenu(name, data)
  local r = async()
...
function cRP.openMainMenu(source)
  local menudata = cRP.buildMenu("main", {player = source})
  menudata.name = "Main menu"
  menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
  cRP.openMenu(source,menudata) -- open the generated menu
end

-- SERVER TUNNEL API

function tcRP.closeMenu(id)
  local source = source
...
function tcRP.validMenuChoice(id,choice,mod)
  local source = source
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tcRP.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tcRP.requestResult(id,ok)
...
function tcRP.openMainMenu()
  cRP.openMainMenu(source)
end
    ...
    for k,v in pairs(cfg.static_menus) do
      ...
      if smenu then
        local function smenu_enter(source)
          local user_id = cRP.getUserId(source)
          if user_id ~= nil and cRP.hasPermissions(user_id,smenu.permissions or {}) then
            -- build static menu
            local menu = cRP.buildMenu("static:"..k, {player=source})
            menu.name=v.title
        ...
        cRPclient._addBlip(source,x,y,z,smenu.blipid,smenu.blipcolor,smenu.title)
        cRPclient._addMarker(source,x,y,z-1,0.7,0.7,0.5,255,226,0,125,150)

        cRP.setArea(source,"cRP:static_menu:"..k,x,y,z,1,1.5,smenu_enter,smenu_leave)
      end
    end
  end
end
...
AddEventHandler("cRP:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    cRPclient._setDiv(source,"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,"")
...
function tcRP.signalVoicePeer(player, data)
  cRPclient._signalVoicePeer(player, source, data)
end

AddEventHandler("cRP:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    -- send peer config
    cRPclient._setPeerConfiguration(source, cfg.voip_peer_configuration)
  end
end)
