...
cRP.prepare("cRP/home_tables", [[
CREATE TABLE IF NOT EXISTS cRP_user_homes(
  user_id INTEGER,
  home VARCHAR(100),
  number INTEGER,
  CONSTRAINT pk_user_homes PRIMARY KEY(user_id),
  CONSTRAINT fk_user_homes_users FOREIGN KEY(user_id) REFERENCES cRP_users(id) ON DELETE CASCADE,
  UNIQUE(home,number)
);
]])

cRP.prepare("cRP/get_address","SELECT home, number FROM cRP_user_homes WHERE user_id = @user_id")
cRP.prepare("cRP/get_home_owner","SELECT user_id FROM cRP_user_homes WHERE home = @home AND number = @number")
cRP.prepare("cRP/rm_address","DELETE FROM cRP_user_homes WHERE user_id = @user_id")
cRP.prepare("cRP/set_address","REPLACE INTO cRP_user_homes(user_id,home,number) VALUES(@user_id,@home,@number)")

-- init
async(function()
  cRP.execute("cRP/home_tables")
end)
...
function cRP.getUserAddress(user_id, cbr)
  local rows = cRP.query("cRP/get_address", {user_id = user_id})
  return rows[1]
end

-- set user address
function cRP.setUserAddress(user_id,home,number)
  cRP.execute("cRP/set_address", {user_id = user_id, home = home, number = number})
end

-- remove user address
function cRP.removeUserAddress(user_id)
  cRP.execute("cRP/rm_address", {user_id = user_id})
end

-- return user_id or nil
function cRP.getUserByAddress(home,number,cbr)
  local rows = cRP.query("cRP/get_home_owner", {home = home, number = number})
  if #rows > 0 then
...
function cRP.findFreeNumber(home,max,cbr)
  local i = 1
  while i <= max do
    if not cRP.getUserByAddress(home,i) then
      return i
    end
    i = i+1
  end
...
function cRP.defHomeComponent(name, oncreate, ondestroy)
  components[name] = {oncreate,ondestroy}
end

function cRP.getHomeSlotPlayers(stype, sid)
end
...
local function leave_slot(user_id,player,stype,sid) -- called when a player leave a slot
  ...
  cRPclient._teleport(player, table.unpack(home.entry_point)) -- already an array of params (x,y,z)
  ...
  for k,v in pairs(cfg.slot_types[stype][sid]) do
    ...
    if name == "entry" then
      -- remove marker/area
      local nid = "cRP:home:slot"..stype..sid
      cRPclient._removeNamedMarker(player,nid)
      cRP.removeArea(player,nid)
    else
...
local function enter_slot(user_id,player,stype,sid) -- called when a player enter a slot
  ...
  for k,v in pairs(cfg.slot_types[stype][sid]) do
    ...
    if name == "entry" then
      ...
      cRPclient._teleport(player, x,y,z) -- already an array of params (x,y,z)

      local nid = "cRP:home:slot"..stype..sid
      cRPclient._setNamedMarker(player,nid,x,y,z-1,0.7,0.7,0.5,0,255,125,125,150)
      cRP.setArea(player,nid,x,y,z,1,1.5,entry_enter,entry_leave)
...
function cRP.accessHome(user_id, home, number)
  ...
  local owner_id = cRP.getUserByAddress(home,number)
...
local function build_entry_menu(user_id, home_name)
  ...
  menu[lang.home.intercom.title()] = {function(player,choice)
    local number = cRP.prompt(player, lang.home.intercom.prompt(), "")
      number = parseInt(number)
      local huser_id = cRP.getUserByAddress(home_name,number)
        if huser_id then
          if huser_id == user_id then -- identify owner (direct home access)
            if not cRP.accessHome(user_id, home_name, number) then
                cRPclient._notify(player,lang.home.intercom.not_available())
              end
          else -- try to access home by asking owner
            local hplayer = cRP.getUserSource(huser_id)
            if hplayer ~= nil then
              local who = cRP.prompt(player,lang.home.intercom.prompt_who(),"")
                cRPclient._notify(player,lang.home.intercom.asked())
                -- request owner to open the door
                if cRP.request(hplayer, lang.home.intercom.request({who}), 30) then
                    cRP.accessHome(user_id, home_name, number)
                  else
                    cRPclient._notify(player,lang.home.intercom.refused())
                  end
            else
              cRPclient._notify(player,lang.home.intercom.refused())
            end
  ...
  menu[lang.home.buy.title()] = {function(player,choice)
    local address = cRP.getUserAddress(user_id)
      if not address then -- check if not already have a home
        local number = cRP.findFreeNumber(home_name, home.max)
          if number then
            if cRP.tryPayment(user_id, home.buy_price) then
              -- bought, set address
              cRP.setUserAddress(user_id, home_name, number)

              cRPclient._notify(player,lang.home.buy.bought())
            else
              cRPclient._notify(player,lang.money.not_enough())
            end
          else
            cRPclient._notify(player,lang.home.buy.full())
          end
      else
        cRPclient._notify(player,lang.home.buy.have_home())
      end
  ...
  menu[lang.home.sell.title()] = {function(player,choice)
    local address = cRP.getUserAddress(user_id)
      if address and address.home == home_name then -- check if already have a home
        ...
        cRP.giveMoney(user_id, home.sell_price)
        cRP.removeUserAddress(user_id)
        cRPclient._notify(player,lang.home.sell.sold())
      else
        cRPclient._notify(player,lang.home.sell.no_home())
      end
  ...
  if user_id then
    for k,v in pairs(cfg.homes) do
      ...
      local function entry_enter(player,area)
        ...
        if user_id and cRP.hasPermissions(user_id,v.permissions or {}) then
          cRP.openMenu(source,build_entry_menu(user_id, k))
        end
      end
      ...
      cRPclient._addBlip(source,x,y,z,v.blipid,v.blipcolor,k)
      cRPclient._addMarker(source,x,y,z-1,0.7,0.7,0.5,0,255,125,125,150)

      cRP.setArea(source,"cRP:home"..k,x,y,z,1,1.5,entry_enter,entry_leave)
    end
  end
end
