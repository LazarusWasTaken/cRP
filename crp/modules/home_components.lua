...
local function chest_create(owner_id, stype, sid, cid, config, data, x, y, z, player)
  local chest_enter = function(player,area)
    ...
    if user_id and user_id == owner_id then
      cRP.openChest(player, "u"..owner_id.."home", config.weight or 200,nil,nil,nil)
    end
  end
  ...
  local nid = "cRP:home:slot"..stype..sid..":chest"
  cRPclient._setNamedMarker(player,nid,x,y,z-1,0.7,0.7,0.5,0,255,125,125,150)
  cRP.setArea(player,nid,x,y,z,1,1.5,chest_enter,chest_leave)
end
...
cRP.defHomeComponent("chest", chest_create, chest_destroy)
...
local function wardrobe_create(owner_id, stype, sid, cid, config, data, x, y, z, player)
  ...
  wardrobe_enter = function(player,area)
    ...
    if user_id and user_id == owner_id then
      ...
      local udata = cRP.getUserDataTable(user_id)
      if udata.cloakroom_idle then
        cRPclient._notify(player,lang.common.wearing_uniform())
      end
      ...
      -- load sets
      local udata = cRP.getUData(user_id, "cRP:home:wardrobe")
      ...
      menu[lang.home.wardrobe.save.title()] = {function(player, choice)
        local setname = cRP.prompt(player, lang.home.wardrobe.save.prompt(), "")
        ...
        if string.len(setname) > 0 then
          ...
          sets[setname] = custom
          -- save to db
          cRP.setUData(user_id,"cRP:home:wardrobe",json.encode(sets))
      ...
      local choose_set = function(player,choice)
        ...
        if custom then
          cRPclient._setCustomization(player,custom)
        end
      end
  ...
  local nid = "cRP:home:slot"..stype..sid..":wardrobe"
  cRPclient._setNamedMarker(player,nid,x,y,z-1,0.7,0.7,0.5,0,255,125,125,150)
  cRP.setArea(player,nid,x,y,z,1,1.5,wardrobe_enter,wardrobe_leave)
end
...
cRP.defHomeComponent("wardrobe", wardrobe_create, wardrobe_destroy)
...
local function gametable_create(owner_id, stype, sid, cid, config, data, x, y, z, player)
  local gametable_enter = function(player,area)
    ...
    if user_id and user_id == owner_id then
      ...
      menu[lang.home.gametable.bet.title()] = {function(player, choice)
        local amount = cRP.prompt(player, lang.home.gametable.bet.prompt(), "")
          amount = parseInt(amount)
          if amount > 0 then
            if cRP.tryPayment(user_id,amount) then
              cRPclient._notify(player,lang.home.gametable.bet.started())
              ...
              local close_bet = function()
                if bet_opened then
                  ...
                  local wuser_id = cRP.getUserId(wplayer)
                  if wuser_id then
                    cRP.giveMoney(wuser_id, bet_total)
                    cRPclient._notify(wplayer,lang.money.received({bet_total}))
                    cRPclient._playAnim(wplayer,true,{{"mp_player_introck","mp_player_int_rock",1}},false)
                  end
                end
              end

              -- send bet request to all nearest players
              local players = cRPclient.getNearestPlayers(player,7)
                local pcount = 0
                for k,v in pairs(players) do
                  ...
                  if nuser_id then -- request
                    Citizen.CreateThread(function() -- non blocking
                      if cRP.request(nplayer,lang.home.gametable.bet.request({amount}), 30) and bet_opened then
                        if cRP.tryPayment(nuser_id,amount) then -- register player bet
                          bet_total = bet_total+amount
                          table.insert(bet_players, nplayer)
                          cRPclient._notify(nplayer,lang.money.paid({amount}))
                        else
                          cRPclient._notify(nplayer,lang.money.not_enough())
                        end
                      end
  ...
  local nid = "cRP:home:slot"..stype..sid..":gametable"
  cRPclient._setNamedMarker(player,nid,x,y,z-1,0.7,0.7,0.5,0,255,125,125,150)
  cRP.setArea(player,nid,x,y,z,1,1.5,gametable_enter,gametable_leave)
end
...
cRP.defHomeComponent("gametable", gametable_create, gametable_destroy)
...
local function itemtr_create(owner_id, stype, sid, cid, config, data, x, y, z, player)
  ...
  if itemtrs[nid] == nil then
    ...
    cRP.setItemTransformer(nid, itemtr)
  else
...
local function itemtr_destroy(owner_id, stype, sid, cid, config, data, x, y, z, player)
  ...
  if itemtrs[nid] ~= nil then
    ...
    if itemtrs[nid] == 0 then
      itemtrs[nid] = nil
      cRP.removeItemTransformer(nid)
    end
  end
end

cRP.defHomeComponent("itemtr", itemtr_create, itemtr_destroy)

-- RADIO

local function radio_create(owner_id, stype, sid, cid, config, data, x, y, z, player)
  local nid = "cRP:home:slot"..stype..sid..":radio"..cid
  -- build menu
  ...
  local function choose(player, choice)
    ...
    local players = cRP.getHomeSlotPlayers(stype, sid) or {}
    for k,v in pairs(players) do
      if data.station then
        cRPclient._setAudioSource(v, nid, data.station, 0.5, rx, ry, rz, 50)
      else
        cRPclient._removeAudioSource(v, nid)
      end
    end
  end
  ...
  cRPclient._setNamedMarker(player,nid,x,y,z-1,0.7,0.7,0.5,0,255,125,125,150)
  cRP.setArea(player,nid,x,y,z,1,1.5,radio_enter,radio_leave)

  if data.station then -- auto load station
    cRPclient._setAudioSource(player, nid, data.station, 0.5, rx, ry, rz, 50)
  end
end
...
local function radio_destroy(owner_id, stype, sid, cid, config, data, x, y, z, player)
  ...
  cRPclient._removeNamedMarker(player,nid)
  cRP.removeArea(player,nid)
  cRPclient._removeAudioSource(player, nid)
end

cRP.defHomeComponent("radio", radio_create, radio_destroy)
