...
namespace cRP
{
  public class MySQL : BaseScript
  {
    ...
    public MySQL()
    {
      Console.WriteLine("[cRP/C#] Load MySQL app.");
      task_id = 0;
      ...
      EventHandlers["cRP:MySQL_query"] += new Action<string,IDictionary<string,object>, int>(e_query);
      EventHandlers["cRP:MySQL_tick"] += new Action(e_tick);
    ...
    public void e_tick()
    {
      ...
      foreach(var pair in tasks){
        ...
        if(task.IsCompleted){
          ...
          if(!task.IsFaulted){ //ok
            ...
            dict["status"] = -1;
            Console.WriteLine("[cRP/C#] query exception "+path+" : "+task.Exception.ToString());
          }
      ...
      foreach(var pair in rmtasks){
        TriggerEvent("cRP:MySQL_task", pair.Key, pair.Value);
      }
    ...
    {
      try{
//      Console.WriteLine("[cRP/C#] create connection "+name);
...
//        Console.WriteLine("[cRP/C#] create command "+path);
      }
      else
        Console.WriteLine("[cRP/C#] connection "+concmd[0]+" not found");
      }catch(Exception e){ Console.WriteLine(e.ToString()); }
    }
...
//            Console.WriteLine("[cRP/C#] do query "+path);

//            Console.WriteLine("[cRP/C#] add params");
            //set parameters
            ...
            if(mode == 0){ //NonQuery
//              Console.WriteLine("[cRP/C#] try non query");
              int affected = await command.ExecuteNonQueryAsync();
//              Console.WriteLine("[cRP/C#] returns");
              dict["affected"] = affected;

              r = (object)dict;
            }
            else if(mode == 1){ //Scalar
//              Console.WriteLine("[cRP/C#] try scalar");
...
//              Console.WriteLine("[cRP/C#] try reader");
              using (var reader = await command.ExecuteReaderAsync())
              {
                var results = new List<Dictionary<string, object>>();

//                Console.WriteLine("[cRP/C#] in reader");
                while (await reader.ReadAsync())
                {
//                  Console.WriteLine("[cRP/C#] read async");
...
//            Console.WriteLine("[cRP/C#] end query "+path);
            connection.mutex.Release();
//            Console.WriteLine("[cRP/C#] released");
          ...
          Console.WriteLine("[cRP/C#] connection/command path "+path+" not found");

//        Console.WriteLine("[cRP/C#] query "+path+" id "+task);
      }
      else
        Console.WriteLine("[cRP/C#] connection/command path "+path+" not found");

      TriggerEvent("cRP:MySQL_taskid", task);
      }catch(Exception e){ Console.WriteLine(e.ToString()); }
    }
    ...
    {
      Console.WriteLine("[cRP/C#] check task "+id);
      Dictionary<string,object> dict = new Dictionary<string,object>();

      TriggerEvent("cRPtestevent", 5);

      Task<object> task = null;
      if(tasks.TryGetValue((uint)id, out task)){
        Console.WriteLine("[cRP/C#] have task "+id);
        if(!task.IsFaulted){
          Console.WriteLine("[cRP/C#] task not faulted "+id);
          if(task.IsCompleted){
            Console.WriteLine("[cRP/C#] send back mysql result to "+id);
            ...
            if(task.Result != null){
              ...
              Console.WriteLine("[cRP/C#] task "+id+" null result");
              dict["status"] = -1;
              tasks.Remove((uint)id);
              return dict;
            }
          ...
          Console.WriteLine("[cRP/C#] task "+id+" faulted: "+task.Exception.ToString());
          tasks.Remove((uint)id);
          dict["status"] = -1;
          return dict;
        }
        ...
        Console.WriteLine("[cRP/C#] task "+id+" missing");
        dict["status"] = -1;
        return dict;
      }
